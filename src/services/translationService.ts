...
export class TranslationService {
  /**
   * Gemini API를 활용하여 단순 텍스트의 오타/줄바꿈을 보정한 후, 지정된 언어로 번역을 수행합니다. (순수 번역 도메인)
    ...
    if (!config.geminiApiKey || config.geminiApiKey === 'your_gemini_api_key_here') {
      ...
      return {
        correctedText: text
          ...
          .replace(/\s+/g, ' ')
          .trim() + " (AI 보정 완료)",
        translatedText: `[Mock Translation - ${langName}] 이 결과는 GEMINI_API_KEY가 등록되지 않아 생성된 순수 번역 모의 텍스트입니다. 원본: "${text.substring(0, 50)}..."`,
        explanation: `이 모의 응답은 순수 번역 Sandbox용입니다. 타겟 언어: ${targetLanguage}, 감지 소스: ${sourceLanguage}`
      };
    }
