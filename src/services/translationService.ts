...
export class TranslationService {
   ...
   * 또한 이민자를 위한 서류 분석(문서 유형, 실천 지침, 법적 의무 및 연계 서류 로드맵)을 생성합니다.
  ...
  translateAndCorrect = async (
    ...
    if (!config.geminiApiKey || config.geminiApiKey === 'your_gemini_api_key_here') {
      ...
      let mockDocType = '기타 일반 문서 (General Document)';
      let mockActionPlan = [
      ...
      let mockNextRoadmap: NextRoadmap = {
...
`;
      ...
      // Markdown Format (기본값)
      content = `# 🌐 AI 구글 렌즈 번역 및 서류 로드맵 보고서
> **📅 생성 일시**: \`${dateStr}\`
> **🎯 대상 언어**: \`${langName}\`
