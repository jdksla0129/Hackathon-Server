...
- **이민자 맞춤형 서류 로드맵, 조치 사항 및 법적 의무 분석 기능 도입 (2026-07-09)**:
  - 이민 행정 서류가 복잡하여 준비 단계를 놓치기 쉬운 사용자 경험(UX) 문제를 혁신하기 위해 번역 API에 서류 연계 로드맵(`nextRoadmap`), 단계별 행동 지침(`actionPlan`), 그리고 출입국관리법·주택임대차보호법 등에 의거한 법적 의무 및 위반 시 과태료 안내(`legalObligations`) 분석 흐름을 도입했습니다.
